...
window.addEventListener("DOMContentLoaded", () => {
  // Wait for the DOM content to be fully loaded before adding the resize event listener
  window.addEventListener("resize", navbarColorOnResize);
});

function navbarColorOnResize() {
  const referenceButtons = document.querySelector('.your-reference-buttons-selector');
  const sidenav = document.querySelector('.your-sidenav-selector');
  ...
  if (window.innerWidth > 1200) {
    if (referenceButtons && referenceButtons.classList.contains('active') && referenceButtons.getAttribute('data-class') === 'bg-transparent') {
      ...
      if (body && !body.classList.contains('dark-version')) {
        sidenav.classList.add('bg-white');
      }
    }
  } else {
